...
function getPersonCallback(personObj){
    personAttrsName = ['name', 'height', 
                       ...
                       'birth_year', 'gender'];
    formattedPersonAttr = formatPersonAttr(personObj)
    generateResidentsRow(personObj, personAttrsName)
}

function formatPersonAttr(personArray){
    if(personArray.height !== 'unknown'){
        personArray.height = personArray.height + " m"
    }
    if(personArray.mass !== 'unknown'){
        personArray.mass = personArray.mass + " kg"
    }
    return personArray
}
